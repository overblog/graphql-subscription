...
GQL;
    ...
    public function testCreateAction(?string $expectedSchemaName): void
    {
        ...
        $this->assertArrayHasKey('extensions', $actual);
        $this->assertRegExp('@^https://graphql.org/subscriptions/[a-zA-Z0-9]{12}@', $actual['extensions']['topic']);
        $actualParseToken = (new Parser())->parse($actual['extensions']['token']);
        $this->assertSame(['subscribe' => [$actual['extensions']['topic']]], (array) $actualParseToken->getClaim('mercure'));
