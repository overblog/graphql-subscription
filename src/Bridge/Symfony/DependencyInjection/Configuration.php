                ...
                ->arrayNode('mercure')
                    ...
                    ->children()
                        ...
                        ->arrayNode('jwt')
                            ->addDefaultsIfNotSet()
                            ->children()
                                ->scalarNode('publish_provider')
                                    ->defaultValue(JwtPublishProvider::class)
                                    ->info('The ID of a service to call to generate the publisher JSON Web Token.')
                                ->end()
                                ->scalarNode('subscribe_provider')
                                    ->defaultValue(JwtSubscribeProvider::class)
                                    ->info('The ID of a service to call to generate the subscriber JSON Web Token.')
                                ->end()
                                ->scalarNode('publish_secret_key')->info('The JWT secret key to use to publish to this hub.')->end()
                                ->scalarNode('subscribe_secret_key')->info('The JWT secret key to use for subscribe.')->end()
                            ->end()
                        ->end()
                    ->end()
                ->end()
